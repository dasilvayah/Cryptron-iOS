...
class HomeScreenViewController: UIViewController, Subscriber, Trackable {
    ...
    private let logo = UIImageView(image: UIImage(named: "LogoGradient")) // Small
    ...
    private let tradeButtonIndex = 2
    private let menuButtonIndex = 1
    ...
    private func setupToolbar() {
        let buttons = [(buyButtonTitle, #imageLiteral(resourceName: "buy"), #selector(buy)),
                       ...
                       (S.HomeScreen.trade, #imageLiteral(resourceName: "trade"), #selector(trade))].map { (title, image, selector) -> UIBarButtonItem in
